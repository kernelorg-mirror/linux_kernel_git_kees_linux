...
{
	if (!(cs->bcs[0].hw.tiger.send = kmalloc_array(NETJET_DMA_TXSIZE, sizeof(unsigned int), GFP_KERNEL | GFP_DMA))) {
		printk(KERN_WARNING
	...
	if (!(cs->bcs[0].hw.tiger.rec = kmalloc_array(NETJET_DMA_RXSIZE, sizeof(unsigned int), GFP_KERNEL | GFP_DMA))) {
		printk(KERN_WARNING
