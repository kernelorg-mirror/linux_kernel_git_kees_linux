...
static char *resolv_usage_page(unsigned page, struct seq_file *f) {
	...
	if (!f) {
		buf = kcalloc(HID_DEBUG_BUFSIZE, sizeof(char), GFP_ATOMIC);
		if (!buf)
		...
		int size)
{
	...
	buf = kmalloc_array(HID_DEBUG_BUFSIZE, sizeof(char), GFP_ATOMIC);
...
static int hid_debug_events_open(struct inode *inode, struct file *file)
{
	...
	if (!(list->hid_debug_buf = kcalloc(HID_DEBUG_BUFSIZE, sizeof(char), GFP_KERNEL))) {
		err = -ENOMEM;
		kfree(list);
		goto out;
	}
