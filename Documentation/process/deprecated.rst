...
open-coded intentional signed wrap-around arithmetic
----------------------------------------------------
...
wrap-around (when using things like the wrap-around sanitizers).
...
please use the add_wrap() helper::
	...
	magic = add_wrap(typeof(magic), counter, rotation);
...
`__signed_wrap`, `__unsigned_wrap`, or `__pointer_wrap` function
attributes.
